...
int main()
{
    system("title Pharmacy Management Program");

    db_response d;
    d.ConnectionFunction();
...
void ShowItemList() {
    ...
    if (!qstate)
    {
        ...
        while ((row = mysql_fetch_row(res)))
        {
            printf("| %-15s | %-15s | %-15s | %-15s | Rs. %-15s | %-15s |\n", row[1], row[2], row[3], row[4], row[5], row[6]);
        }
        printf("-------------------------------------------------------------------------------------------------------------\n");
    }
    else
